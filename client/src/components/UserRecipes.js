...
let userRecipes = []
userRecipes = Object.values(allRecipes).filter(item=>item.createdBy === userData.user._id && item.status === 'active' )

for(let i=0; i<userRecipes.length;i++){
  //create array of object for table columns
    recipes.push({
        status: userRecipes[i].status,
        title: userRecipes[i].title,
        //added here to enable sorting by rating values using redux
        //and lodash orderBy function
        ratingNum: [userRecipes[i].rating],
        rating: <div index={i}>
        <RatingStars size={25} 
        edit={false} 
        rating={Number(userRecipes[i].rating)} 
        />
        </div>,
        date: dateFormat(new Date(userRecipes[i].created), "dd.mm.yyyy hh:mm"),
        category: userRecipes[i].category,
        actions: <div key={userRecipes[i]._id}>
           {/* action buttons */}
                        ...
                        <svg onClick={()=>edit(userRecipes[i]._id)} xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" style={{marginLeft:'20px'}}  className="bi bi-pen" viewBox="0 0 16 16">
                            <path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
                        </svg>
          
                        <svg onClick={()=>remove(userRecipes[i]._id)} xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" style={{marginLeft:'20px'}} className="bi bi-trash3" viewBox="0 0 16 16">
  ...
  order: 'desc' 
    ...
    return (
        <Container className={'justify-content-center'}>
          ...
          ? _.orderBy(recipes,[sortRating.sortBy],[sortRating.order]) 
          : recipes} 
          columns={ columns } 
          striped />
      </Container>
